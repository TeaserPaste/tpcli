...
use crate::cli_args::{
    CloneArgs, ConfigArgs, ConfigCmd, CopyArgs, CreateArgs, DeleteArgs, EditArgs, ListArgs,
    RestoreArgs, RunArgs, SearchArgs, StarArgs, UpdateArgs, UserArgs, UserCmd, ViewArgs,
};
...
pub fn handle_stats(token: Option<String>) -> Result<()> {
    ...
    table.set_header(vec![
        ...
        "Private",
        "Deleted (In Trash)",
    ]); // Assuming deleted logic matches
    table.add_row(vec![
...
pub fn handle_upgrade() -> Result<()> {
    ...
    if backup_exe.exists() {
        if let Err(e) = fs::remove_file(&backup_exe) {
            ...
            eprintln!("{} Failed to remove old backup file: {}", "Warning:".yellow(), e);
        }
    }
    ...
    if let Err(e) = fs::rename(&current_exe, &backup_exe) {
        return Err(anyhow!("Failed to rename current executable (locked?): {}", e));
    }
    ...
    match status_result {
        ...
        _ => {
            ...
            if let Err(e) = fs::rename(&backup_exe, &current_exe) {
                ...
                eprintln!("Your original executable is located at: {}", backup_exe.display());
                eprintln!("Error details: {}", e);
                return Err(anyhow!("Critical rollback failure. Manual intervention required."));
            } else {
                println!("{}", "Rollback successful. Your installation is intact.".yellow());
            }
...
pub fn handle_run(args: RunArgs, token: Option<String>) -> Result<()> {
    ...
    if output.status.success() {
        ...
        if args.on_error_paste {
            ...
            match client.request::<CreateSnippetRequest, crate::types::CreateSnippetResponse>(
                "/createSnippet",
                "POST",
